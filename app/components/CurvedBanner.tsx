...
export function CurvedBanner({ image, title, subtitle, tag = 'JUST IN', color = C.pink, imageFit = 'cover', overlay = false, copyAlign = 'left', copyTop = false, textColor, hideCopy = false, showTagOnly = false, onPress }: CurvedBannerProps) {
  ...
  return <MotionPressable onPress={onPress} contentStyle={{ flex: 1 }} style={{ height: 250, backgroundColor: color, borderRadius: 34, overflow: 'hidden', position: 'relative' }}>
    <Image source={source} cachePolicy="memory-disk" contentFit={imageFit} style={overlay ? { position: 'absolute', left: 0, right: 0, bottom: -8, width: '100%', height: showTagOnly ? '78%' : '100%', opacity: 1 } : { position: 'absolute', right: imageFit === 'contain' ? -8 : -20, bottom: 0, width: imageFit === 'contain' ? '60%' : '62%', height: imageFit === 'contain' ? '96%' : '100%', opacity: 0.96 }} />
    {(!hideCopy || showTagOnly) && <View style={overlay ? { position: 'absolute', left: 0, top: 0, width: '100%', height: showTagOnly ? 72 : 96, padding: 16, paddingTop: 14, justifyContent: 'flex-start', alignItems, backgroundColor: color } : { position: 'absolute', left: 0, top: 0, bottom: 0, width: '61%', padding: 22, paddingTop: copyTop ? 60 : 22, justifyContent: copyTop ? 'flex-start' : 'center' }}>
