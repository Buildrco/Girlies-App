...
type ChromeVisibility = { visibility: Animated.Value; onScroll: (event: NativeSyntheticEvent<NativeScrollEvent>) => void; reset: () => void };
...
export function ChromeVisibilityProvider({ children }: { children: React.ReactNode }) {
  ...
  const onScroll = useCallback((event: NativeSyntheticEvent<NativeScrollEvent>) => {
    ...
    else if (delta !== 0) currentVisibility.current = Math.max(0, Math.min(1, currentVisibility.current - delta / HIDE_DISTANCE));
    visibility.setValue(currentVisibility.current);
    lastOffset.current = offset;
  }, [visibility]);
  const reset = useCallback(() => { lastOffset.current = 0; currentVisibility.current = 1; visibility.setValue(1); }, [visibility]);
  return <ChromeVisibilityContext.Provider value={{ visibility, onScroll, reset }}>{children}</ChromeVisibilityContext.Provider>;
}

export function useChromeVisibility() {
  const value = useContext(ChromeVisibilityContext);
  if (!value) throw new Error('useChromeVisibility must be used inside ChromeVisibilityProvider');
  return value;
}
...
export function BottomNav() {
  ...
  const moveHighlight = useCallback((label:string) => {
    ...
    if (!initialized.current) {
      highlightLeft.setValue(target.left); highlightWidth.setValue(target.width); lastTarget.current = target; initialized.current = true; return;
    }
    const previous = lastTarget.current;
    if (previous && previous.left === target.left && previous.width === target.width) return;
    const startCenter = (previous?.left ?? target.left) + (previous?.width ?? target.width) / 2;
    const targetCenter = target.left + target.width / 2;
    const stretchWidth = Math.min(145, Math.max(previous?.width ?? 38, target.width) + Math.abs(targetCenter - startCenter) * 0.36);
    const stretchLeft = (startCenter + targetCenter) / 2 - stretchWidth / 2;
    animation.current?.stop();
    animation.current = Animated.sequence([
      Animated.parallel([
        Animated.timing(highlightLeft,{toValue:stretchLeft,duration:190,easing:Easing.inOut(Easing.cubic),useNativeDriver:false}),
        Animated.timing(highlightWidth,{toValue:stretchWidth,duration:190,easing:Easing.out(Easing.cubic),useNativeDriver:false}),
      ]),
      Animated.parallel([
        Animated.timing(highlightLeft,{toValue:target.left,duration:210,easing:Easing.out(Easing.cubic),useNativeDriver:false}),
        Animated.timing(highlightWidth,{toValue:target.width,duration:210,easing:Easing.out(Easing.cubic),useNativeDriver:false}),
      ]),
    ]);
  ...
  useEffect(() => { if (currentActive) moveHighlight(currentActive); }, [currentActive, moveHighlight]);
  ...
  return <Animated.View style={[styles.nav,{transform:[{translateY}]}]}>
    ...
    {items.map(([icon,label,path],index) => {
      ...
      return <MotionPressable key={label} onLayout={event => { positions.current[label] = { x:event.nativeEvent.layout.x, width:event.nativeEvent.layout.width }; if (isActive) moveHighlight(label); }} onPress={() => goToTab(label,path,index)} style={styles.slot}>
        <View style={styles.item}><I name={icon} size={23} color={isActive ? C.pink : C.ink} filled={isActive}/>{isActive && <Text style={styles.label}>{label}</Text>}</View>
      </MotionPressable>;
    })}
  </Animated.View>;
}
