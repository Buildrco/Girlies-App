...
import { createMarketplaceAnnouncement, createService, type MediaItem } from '../lib/social';
...
export default function ServiceEditor() {
  ...
  async function pickImages() {
    ...
    if (!result.canceled) setMedia(current => [...current, ...result.assets.map(asset => ({ uri: asset.uri, type: 'image' as const, name: asset.fileName || 'service.jpg', mimeType: asset.mimeType || 'image/jpeg' }))].slice(0, 4));
  }
  async function save() {
    if (!name.trim() || !price.trim()) { Alert.alert('Complete your service', 'Add a service name and price.'); return; }
    try {
      setSaving(true);
      const service = await createService({ name, description, category, price: Number(price), durationMinutes: Number(duration) || 60, deliveryOptions, filters: {}, media });
  ...
  return <SafeAreaView style={s.safe}><View style={s.flex}><View style={s.top}><Pressable onPress={() => router.back()}><I name="back" size={30} /></Pressable><Text style={s.title}>Add service</Text><Pressable onPress={save} disabled={saving}><Text style={s.save}>{saving ? 'Saving…' : 'Save'}</Text></Pressable></View><ScrollView contentContainerStyle={s.scroll}>
    <Text style={s.label}>Service photos</Text><ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={s.mediaRow}>{media.map((item, index) => <View key={`${item.uri}-${index}`} style={s.mediaWrap}><Image source={{ uri: item.uri }} style={s.preview} /><Pressable onPress={() => setMedia(current => current.filter((_, itemIndex) => itemIndex !== index))} style={s.remove}><Text style={s.removeText}>×</Text></Pressable></View>)}{media.length < 4 && <Pressable onPress={pickImages} style={s.addPhoto}><I name="plus" size={25} color={C.pink} /><Text style={s.addText}>Add photo</Text></Pressable>}</ScrollView>
