...
export default function EditProfile() {
  ...
  const updateLocationText = (kind: 'country' | 'area' | 'town', value: string) => {
    ...
    if (value.trim()) { sheetY.setValue(0); setPicker(kind); }
    else setPicker(null);
  };
  ...
  return <SafeAreaView style={s.safe}><KeyboardAvoidingView style={s.flex} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
    ...
    <ScrollView contentContainerStyle={s.scroll} keyboardShouldPersistTaps="handled">
      ...
      <View style={s.row}><View style={s.half}><LocationField label="Country" value={country} placeholder="Type a country" onChangeText={value => updateLocationText('country', value)} /></View><View style={s.half}><LocationField label="Area" value={area} placeholder={country ? 'Type an area' : 'Choose country first'} disabled={!country || areasLoading} onChangeText={value => updateLocationText('area', value)} /></View></View>
      <LocationField label="Town / city" value={town} placeholder={area ? 'Type a town or city' : 'Choose area first'} disabled={!area} onChangeText={value => updateLocationText('town', value)} />
      ...
      <View style={s.preference}><View style={{ flex: 1 }}><Text style={s.preferenceTitle}>Autoplay videos</Text><Text style={s.preferenceText}>Videos play with sound automatically in your feed.</Text></View><Switch value={videoAutoplay} onValueChange={setVideoAutoplay} trackColor={{ false: C.line, true: C.rose }} thumbColor={videoAutoplay ? C.pink : '#FFF'} /></View>
  ...
  <Modal visible={Boolean(picker)} transparent animationType="none" onRequestClose={closePicker}>
     <View style={s.modalBackdrop}><Pressable style={StyleSheet.absoluteFill} onPress={closePicker} /><Animated.View {...sheetResponder.panHandlers} style={[s.sheet, { transform: [{ translateY: sheetY }] }]}><View style={s.sheetHandle} /><View style={s.sheetTop}><Text style={s.sheetTitle}>{title}</Text><Pressable onPress={closePicker}><I name="back" size={24} /></Pressable></View>{picker === 'area' && areasLoading && country.trim().toLowerCase() !== 'ghana' ? <ActivityIndicator color={C.pink} style={{ margin: 24 }} /> : <ScrollView keyboardShouldPersistTaps="handled" style={s.options}>{items.filter(item => item.toLowerCase().includes(query.trim().toLowerCase())).slice(0, 100).map(item => <Pressable key={item} onPress={() => select(item)} style={s.option}><Text style={s.optionText}>{item}</Text></Pressable>)}</ScrollView>}</Animated.View>
     </View>
  </Modal>
  </SafeAreaView>;
}
...
const s = StyleSheet.create({
   safe:{flex:1,backgroundColor:C.bg},flex:{flex:1},center:{flex:1,alignItems:'center',justifyContent:'center'},top:{height:65,paddingHorizontal:18,flexDirection:'row',alignItems:'center',justifyContent:'space-between',borderBottomWidth:1,borderBottomColor:C.line},title:{fontSize:20,fontWeight:'900'},save:{color:C.pink,fontSize:14,fontWeight:'900'},scroll:{padding:20,paddingBottom:50},avatarButton:{alignSelf:'center',position:'relative',marginTop:5},avatar:{width:92,height:92,borderRadius:46},camera:{position:'absolute',right:0,bottom:0,width:30,height:30,borderRadius:15,backgroundColor:C.pink,alignItems:'center',justifyContent:'center',borderWidth:3,borderColor:C.bg},helper:{textAlign:'center',color:C.muted,fontSize:12,marginTop:9},field:{marginTop:18},label:{fontSize:13,fontWeight:'900',marginBottom:7},input:{minHeight:52,borderRadius:18,backgroundColor:'#FFF',paddingHorizontal:16,paddingVertical:14,borderWidth:1,borderColor:C.line,fontSize:15,color:C.ink},textarea:{minHeight:100,textAlignVertical:'top'},row:{flexDirection:'row',gap:10},half:{flex:1},note:{fontSize:12,lineHeight:17,color:C.muted,marginTop:18},preference:{marginTop:20,padding:16,borderRadius:20,backgroundColor:'#FFF',borderWidth:1,borderColor:C.line,flexDirection:'row',alignItems:'center',gap:12},preferenceTitle:{fontWeight:'900',fontSize:14},preferenceText:{color:C.muted,fontSize:11,lineHeight:16,marginTop:4},inputRow:{minHeight:52,borderRadius:18,backgroundColor:'#FFF',borderWidth:1,borderColor:C.line,flexDirection:'row',alignItems:'center'},locationInput:{flex:1,minHeight:52,paddingHorizontal:14,fontSize:14,color:C.ink},clear:{width:34,height:34,borderRadius:17,backgroundColor:C.rose,alignItems:'center',justifyContent:'center',marginRight:8},clearText:{fontSize:24,lineHeight:26,color:C.pink},selectorDisabled:{opacity:.55},modalBackdrop:{flex:1,justifyContent:'flex-end',backgroundColor:'rgba(23,19,24,.32)'},sheet:{maxHeight:'82%',backgroundColor:C.bg,borderTopLeftRadius:30,borderTopRightRadius:30,padding:18},sheetHandle:{alignSelf:'center',width:42,height:5,borderRadius:3,backgroundColor:C.line,marginBottom:12},sheetTop:{flexDirection:'row',alignItems:'center',justifyContent:'space-between'},sheetTitle:{fontSize:20,fontWeight:'900'},options:{marginTop:10},option:{minHeight:50,paddingHorizontal:12,borderBottomWidth:1,borderBottomColor:C.line,justifyContent:'center'},optionText:{fontSize:14,fontWeight:'700'}
});