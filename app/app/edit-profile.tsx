...
export default function EditProfile() {
  ...
  return <SafeAreaView style={s.safe}><KeyboardAvoidingView style={s.flex} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
    ...
    <ScrollView contentContainerStyle={s.scroll} keyboardShouldPersistTaps="handled">
      ...
      <View style={s.row}><View style={s.half}><Selector label="Country" value={country} placeholder="Select country" onPress={() => { sheetY.setValue(0); setQuery(''); setPicker('country'); }} /></View><View style={s.half}><Selector label="Area" value={area} placeholder={country ? 'Select area' : 'Choose country first'} disabled={!country || areasLoading} onPress={() => { sheetY.setValue(0); setQuery(''); setPicker('area'); }} /></View></View>
      <Selector label="Town / city" value={town} placeholder={area ? 'Select town or city' : 'Choose area first'} disabled={!area} onPress={() => { sheetY.setValue(0); setQuery(''); setPicker('town'); }} />
  ...
  <Modal visible={Boolean(picker)} transparent animationType="none" onRequestClose={closePicker}>
     <View style={s.modalBackdrop}><Pressable style={StyleSheet.absoluteFill} onPress={closePicker} /><Animated.View {...sheetResponder.panHandlers} style={[s.sheet, { transform: [{ translateY: sheetY }] }]}><View style={s.sheetHandle} /><View style={s.sheetTop}><Text style={s.sheetTitle}>{title}</Text><Pressable onPress={closePicker}><I name="back" size={24} /></Pressable></View><TextInput autoFocus value={query} onChangeText={value => { setQuery(value); if (!value.trim()) { if (picker === 'country') { setCountry(''); setArea(''); setTown(''); } else if (picker === 'area') { setArea(''); setTown(''); } else if (picker === 'town') setTown(''); } }} placeholder={picker === 'area' ? 'Type an area, e.g. Awoshie' : 'Search'} placeholderTextColor={C.muted} style={s.searchInput} />{picker === 'area' && areasLoading && country.trim().toLowerCase() !== 'ghana' ? <ActivityIndicator color={C.pink} style={{ margin: 24 }} /> : <ScrollView keyboardShouldPersistTaps="handled" style={s.options}>{items.filter(item => item.toLowerCase().includes(query.trim().toLowerCase())).slice(0, 100).map(item => <MotionPressable key={item} onPress={() => select(item)} style={s.option}><Text style={s.optionText}>{item}</Text></MotionPressable>)}</ScrollView>}</Animated.View>
     </View>
  </Modal>
  </SafeAreaView>;
}
