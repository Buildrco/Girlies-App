...
export default function SellerStudio() {
  ...
  return <SafeAreaView style={s.safe}><ScrollView contentContainerStyle={s.scroll} showsVerticalScrollIndicator={false}>
    ...
    {!products.length && !error && <View style={s.emptyCard}><Text style={s.spark}>✦</Text><Text style={s.emptyTitle}>Your first product belongs here.</Text><Text style={s.emptyText}>Add a photo, description, price and optional bid price. Published products appear in Shop and Fresh finds.</Text><Pressable style={s.primary} onPress={() => router.push('/product-editor')}><Text style={s.primaryText}>Add your first product</Text></Pressable></View>}
     ...
     <View style={s.productList}>{services.map(service => <View key={service.id} style={s.product}><View style={s.serviceImage}>{service.image_urls?.[0] ? <Image source={{ uri: service.image_urls[0] }} style={s.serviceImage} /> : <I name="spark" size={22} color={C.pink} />}</View><View style={s.productCopy}><Text style={s.productName} numberOfLines={1}>{service.name}</Text><Text style={s.productMeta}>{service.category} · {service.duration_minutes} min</Text><Text style={s.productPrice}>GH₵ {Number(service.price).toFixed(0)}</Text></View><Pressable onPress={() => confirmDeleteService(service)}><Text style={s.deleteLabel}>Delete</Text></Pressable></View>)}</View>
