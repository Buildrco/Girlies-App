...
export default function SellerStudio() {
  ...
  return <SafeAreaView style={s.safe}><ScrollView contentContainerStyle={s.scroll} showsVerticalScrollIndicator={false}>
    ...
    <View style={s.productList}>{products.map(product => <Pressable key={product.id} style={s.product} onPress={() => router.push({ pathname: '/product-editor', params: { id: product.id } })}>{product.image_urls?.[0] ? <Image source={{ uri: product.image_urls[0] }} style={s.productImage} /> : <View style={[s.productImage, s.placeholder]}><I name="shop" size={24} color={C.pink} /></View>}<View style={s.productCopy}><Text style={s.productName} numberOfLines={1}>{product.name}</Text><Text style={s.productMeta}>{product.category} · {product.stock_status === 'out_of_stock' ? 'Out of stock' : `${product.stock} in stock`}</Text><Text style={s.productPrice}>GH₵ {Number(product.price).toFixed(0)}</Text></View><Text style={s.editLabel}>Edit</Text></Pressable>)}</View>
...
const s = StyleSheet.create({
  ...
  productList: { gap: 8 }, product: { minHeight: 82, padding: 10, borderRadius: 23, backgroundColor: '#FFF', flexDirection: 'row', alignItems: 'center', gap: 11 }, productImage: { width: 62, height: 62, borderRadius: 17 }, placeholder: { backgroundColor: C.cream, alignItems: 'center', justifyContent: 'center' }, productCopy: { flex: 1 }, productName: { fontSize: 14, fontWeight: '900' }, productMeta: { color: C.muted, fontSize: 11, marginTop: 4 }, productPrice: { color: C.pink, fontSize: 13, fontWeight: '900', marginTop: 4 }, editLabel: { color: C.pink, fontWeight: '900', fontSize: 12 },
});