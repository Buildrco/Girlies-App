...
const secondarySections = [
  ['referrals', 'Referrals', 'arrow'], ['campaigns', 'Campaigns', 'chart'], ['discounts', 'Discounts', 'gift'], ['announcements', 'Announcements', 'chat'],
] as const;
...
export default function SellerStudio() {
  ...
  const quick = [['Orders', String(live.orders), 'receipt'], ['Visitors', String(live.visitors), 'people'], ['Purchases', String(live.purchases), 'bag'], ['Analytics', String(live.views), 'chart']] as const;
  return <SafeAreaView style={s.safe}><ScrollView contentContainerStyle={s.scroll} showsVerticalScrollIndicator={false}>
    ...
    <View style={s.sectionHead}><View><Text style={s.eyebrow}>SHOP OVERVIEW</Text><Text style={s.sectionTitle}>Your business at a glance</Text></View></View>
    ...
    <View style={s.quickGrid}>{quick.map(([label, value, icon]) => <Pressable key={label} style={s.quickCard} onPress={() => router.push(label === 'Analytics' ? '/seller-studio/analytics' : label === 'Orders' ? '/seller-studio/orders' : '/seller-studio/analytics')}><View style={s.icon}><I name={icon} size={19} color="#FFF" filled /></View><Text style={s.quickValue}>{value}</Text><Text style={s.quickLabel}>{label}</Text></Pressable>)}</View>
