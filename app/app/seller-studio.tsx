...
export default function SellerStudio() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      setStore(nextStore);
      if (nextStore) setProducts(await getProducts(50, { storeId: nextStore.id }));
  ...
  return <SafeAreaView style={s.safe}><ScrollView contentContainerStyle={s.scroll} showsVerticalScrollIndicator={false}>
    ...
    <View style={s.actions}><Pressable style={s.actionPrimary} onPress={() => router.push('/product-editor')}><I name="plus" size={20} color="#FFF" /><Text style={s.actionPrimaryText}>Add product</Text></Pressable><Pressable style={s.actionSecondary} onPress={() => router.push('/shop-editor')}><I name="settings" size={19} color={C.ink} /><Text style={s.actionSecondaryText}>Edit shop</Text></Pressable></View>
    <View style={s.stats}><View style={s.stat}><Text style={s.num}>{products.length}</Text><Text style={s.label}>Products</Text></View><View style={s.stat}><Text style={s.num}>Live</Text><Text style={s.label}>Shop status</Text></View><View style={s.stat}><Text style={s.num}>GH₵</Text><Text style={s.label}>Currency</Text></View></View>
    ...
    <View style={s.productList}>{products.map(product => <Pressable key={product.id} style={s.product} onPress={() => router.push({ pathname: '/product', params: { id: product.id } })}>{product.image_urls?.[0] ? <Image source={{ uri: product.image_urls[0] }} style={s.productImage} /> : <View style={[s.productImage, s.placeholder]}><I name="shop" size={24} color={C.pink} /></View>}<View style={s.productCopy}><Text style={s.productName} numberOfLines={1}>{product.name}</Text><Text style={s.productMeta}>{product.category} · {product.stock} in stock</Text><Text style={s.productPrice}>GH₵ {Number(product.price).toFixed(0)}</Text></View><I name="arrow" size={18} color={C.muted} /></Pressable>)}</View>
...
const s = StyleSheet.create({
  ...
  productList: { gap: 8 }, product: { minHeight: 82, padding: 10, borderRadius: 23, backgroundColor: '#FFF', flexDirection: 'row', alignItems: 'center', gap: 11 }, productImage: { width: 62, height: 62, borderRadius: 17 }, placeholder: { backgroundColor: C.cream, alignItems: 'center', justifyContent: 'center' }, productCopy: { flex: 1 }, productName: { fontSize: 14, fontWeight: '900' }, productMeta: { color: C.muted, fontSize: 11, marginTop: 4 }, productPrice: { color: C.pink, fontSize: 13, fontWeight: '900', marginTop: 4 },
});