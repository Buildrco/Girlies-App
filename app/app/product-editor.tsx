...
export default function ProductEditor() {
  ...
  return <SafeAreaView style={s.safe}><View style={s.flex}>
    ...
    <ScrollView contentContainerStyle={s.scroll} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
      <View style={s.mediaPicker}><ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={s.mediaRow}>{[...existingImages.map(uri => ({ uri })), ...media].slice(0, 4).map((item, index) => <Image key={`${item.uri}-${index}`} source={{ uri: item.uri }} style={s.preview} />)}{existingImages.length + media.length < 4 && <Pressable onPress={pickImage} style={s.addPhoto}><I name="plus" size={28} color={C.pink} /><Text style={s.addPhotoText}>Add photo</Text></Pressable>}</ScrollView>{!existingImages.length && !media.length && <Text style={s.mediaText}>Add up to 4 product photos. You can upload one or fill all four.</Text>}</View>
