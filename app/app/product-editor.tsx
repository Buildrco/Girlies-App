...
export default function ProductEditor() {
  ...
  return <SafeAreaView style={s.safe}><View style={s.flex}>
    ...
    <ScrollView contentContainerStyle={s.scroll} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
      <Pressable onPress={pickImage} style={s.mediaPicker}>{existingImages.length + media.length ? <ScrollView horizontal showsHorizontalScrollIndicator={false}>{[...existingImages.map(uri => ({ uri })), ...media].slice(0, 4).map((item, index) => <Image key={`${item.uri}-${index}`} source={{ uri: item.uri }} style={s.preview} />)}</ScrollView> : <><View style={s.camera}><I name="camera" size={28} color={C.pink} /></View><Text style={s.mediaTitle}>Add up to 4 product photos</Text><Text style={s.mediaText}>The first photo is shown in the shop.</Text></>}</Pressable>
      ...
      <Pressable style={s.primary} onPress={save} disabled={saving}><Text style={s.primaryText}>{saving ? 'Saving…' : editingId ? 'Save product changes' : 'Publish product'}</Text><I name="arrow" size={19} color="#FFF" /></Pressable>
    </ScrollView>
  </View></SafeAreaView>;
}
