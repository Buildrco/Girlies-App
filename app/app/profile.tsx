...
import { ActivityIndicator, Alert, Animated, SafeAreaView, ScrollView, View, Text, Pressable, Image, StyleSheet, TextInput, useWindowDimensions } from 'react-native';
...
type Post = { id: string; author_id: string; body: string; media_urls: string[] | null; metadata?: any; created_at: string };

const VIDEO_URL_PATTERN = /\.(mp4|mov|m4v|webm|m3u8)(?:[?#]|$)/i;
function getMediaType(post: Post, url: string, index: number): 'image' | 'video' {
...
export default function Profile() {
  ...
  const tabWidth = Math.max(1, (width - 36) / 3);
  ...
  const load = useCallback(async (userId: string | null) => {
    ...
    const [currentProfile, postsResult, store] = await Promise.all([
      getCurrentProfile(),
       supabase.from('posts').select('id,author_id,body,media_urls,metadata,created_at').eq('author_id', userId).order('created_at', { ascending: false }),
  ...
  useEffect(() => {
    Animated.timing(tabUnderline, { toValue: tabIndex, duration: 430, useNativeDriver: true }).start();
  }, [tabIndex, tabUnderline]);
  ...
  return <SafeAreaView style={s.safe}>
    <ScrollView contentContainerStyle={s.scroll} onScroll={onScroll} scrollEventThrottle={16}>
       <View style={s.top}><Text style={s.h}>Profile</Text><Pressable hitSlop={10} onPress={() => router.push('/settings')}><I name="settings" size={24} /></Pressable></View>
  ...
  const s=StyleSheet.create({safe:{flex:1,backgroundColor:C.bg},center:{flex:1,alignItems:'center',justifyContent:'center',padding:24},scroll:{padding:18,paddingBottom:120},top:{height:66,paddingTop:8,flexDirection:'row',justifyContent:'space-between',alignItems:'center'},h:{fontSize:21,fontWeight:'900'},cover:{height:175,borderRadius:34,backgroundColor:C.sun,overflow:'hidden',position:'relative'},coverShape:{position:'absolute',width:250,height:250,borderRadius:130,right:-40,bottom:-100,backgroundColor:C.rose},avatarWrap:{position:'absolute',left:20,bottom:17,borderWidth:5,borderColor:'#FFF',borderRadius:50},info:{paddingTop:15},name:{fontSize:24,fontWeight:'900'},handle:{fontSize:13,color:C.muted,marginTop:3},bio:{fontSize:14,fontWeight:'600',marginTop:9},place:{fontSize:13,color:C.muted,marginTop:7,flexDirection:'row',alignItems:'center'},links:{flexDirection:'row',flexWrap:'wrap',gap:8,marginTop:8},link:{fontSize:13,color:C.pink,fontWeight:'800'},join:{fontSize:12,color:C.muted,marginTop:5},stats:{flexDirection:'row',gap:35,marginTop:16},num:{fontSize:17,fontWeight:'900'},label:{fontSize:12,color:C.muted,marginTop:2},actions:{flexDirection:'row',gap:9,marginTop:15},edit:{flex:1,height:43,borderRadius:22,backgroundColor:'#FFF',alignItems:'center',justifyContent:'center',borderWidth:1,borderColor:C.line},shopBtn:{flex:1,height:43,borderRadius:22,backgroundColor:C.ink,alignItems:'center',justifyContent:'center',flexDirection:'row',gap:7,paddingHorizontal:18},tabs:{height:49,flexDirection:'row',marginTop:20,borderBottomWidth:1,borderBottomColor:C.line,position:'relative'},tab:{flex:1,alignItems:'center',justifyContent:'center',paddingVertical:13},tabUnderline:{position:'absolute',bottom:-1,left:0,height:3,borderRadius:2,backgroundColor:C.pink},posts:{gap:12,marginTop:12},post:{backgroundColor:'#FFF',borderRadius:24,padding:14,borderWidth:1,borderColor:C.line},postHeader:{flexDirection:'row',alignItems:'center',gap:9,marginBottom:10},postName:{fontSize:13,fontWeight:'900'},postMeta:{fontSize:10,color:C.muted,marginTop:2},postBody:{fontSize:14,lineHeight:20,fontWeight:'600',marginBottom:10},postImage:{width:'100%',height:230,borderRadius:18,marginTop:6},postActions:{flexDirection:'row',alignItems:'center',justifyContent:'space-between',paddingTop:12},action:{flexDirection:'row',alignItems:'center',gap:5},actionText:{fontSize:12},commentBox:{marginTop:10,padding:8,borderRadius:20,backgroundColor:C.bg,flexDirection:'row',alignItems:'center'},commentInput:{flex:1,paddingHorizontal:10,paddingVertical:8},fab:{position:'absolute',right:23,bottom:88,width:55,height:55,borderRadius:28,backgroundColor:C.pink,alignItems:'center',justifyContent:'center',elevation:8},grid:{flexDirection:'row',flexWrap:'wrap',gap:5,marginTop:12},tile:{width:'32.3%',height:145,borderRadius:15},shopGrid:{flexDirection:'row',flexWrap:'wrap',gap:8,marginTop:12},product:{width:'48%',backgroundColor:'#FFF',borderRadius:20,padding:9},productImage:{height:135,width:'100%',borderRadius:15},productPlaceholder:{backgroundColor:C.cream,alignItems:'center',justifyContent:'center'},productName:{fontSize:13,fontWeight:'900',marginTop:8},productPrice:{fontSize:12,color:C.pink,fontWeight:'900',marginTop:4},serviceList:{gap:8,marginTop:12},service:{padding:12,borderRadius:20,backgroundColor:'#FFF'},serviceImage:{width:'100%',height:150,borderRadius:15,marginBottom:3},empty:{padding:50,alignItems:'center'},emptyTitle:{fontSize:19,fontWeight:'900',marginTop:8},emptyText:{fontSize:13,color:C.muted,marginTop:4,textAlign:'center'},refer:{marginTop:12,padding:20,borderRadius:28,backgroundColor:C.lilac},referK:{fontSize:10,fontWeight:'900',letterSpacing:1.2},referTitle:{fontSize:22,fontWeight:'900'},referLink:{fontSize:12,fontWeight:'900',marginTop:12}})
