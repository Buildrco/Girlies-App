...
import { getPostLikeState, getProducts, getSessionUser, ProductRecord, setFollow, setPostLike, StoreRecord } from '../lib/social';
...
export default function Home() {
  ...
  useFocusEffect(useCallback(() => {
    ...
    async function loadLiveState() {
      try {
        ...
        const [storesResult, productsResult, postResult] = await Promise.all([
          supabase.from('stores').select('id,owner_id,name,slug,description,rating,verification_status').order('created_at', { ascending: false }).limit(10),
          getProducts(10),
        ...
        const ownerIds = [...new Set((storesResult.data || []).map(row => row.owner_id).filter(Boolean))];
        const { data: owners, error: ownersError } = ownerIds.length
          ? await supabase.from('profiles').select('id,display_name,handle,bio,avatar_url,verified,followers_count').in('id', ownerIds)
          : { data: [], error: null as any };
        if (ownersError) throw ownersError;
        const ownerMap = new Map((owners || []).map((owner: any) => [owner.id, owner]));
        const liveStores = (storesResult.data || []).map((store: any) => ({ ...store, owner: ownerMap.get(store.owner_id) || null })) as StoreRecord[];
        ...
        const [profileResult, followResult, likeResult, countResult, commentsResult, sharesResult] = await Promise.all([
          ...
          me ? supabase.from('follows').select('following_id').eq('follower_id', me.id).in('following_id', liveStores.map(store => store.owner_id)) : Promise.resolve({ data: [], error: null }),
        ...
        const followedIds = new Set((followResult.data || []).map(row => row.following_id));
         setFollowed(new Set(liveStores.map(store => store.owner_id).filter((ownerId): ownerId is string => Boolean(ownerId && followedIds.has(ownerId)))));
        setLiked(likeResult);
        setHomeLikeCount(countResult.count || 0);
        setHomeCommentCount(commentsResult.count || 0);
        setHomeShareCount(sharesResult.count || 0);
      } catch (error) {
        if (active) setLoadError(error instanceof Error ? error.message : 'Could not load live activity.');
      }
    }
    void loadLiveState();
    return () => { active = false; };
  ...
  async function toggleSeller(targetId: string) {
    ...
    if (followingBusy.has(targetId)) return;
    const next = !followed.has(targetId);
    setFollowingBusy(current => new Set(current).add(targetId));
    try {
      const verified = await setFollow(targetId, next);
      setFollowed(current => { const copy = new Set(current); if (verified) copy.add(targetId); else copy.delete(targetId); return copy; });
    } catch (error) {
      setLoadError(error instanceof Error ? error.message : 'Could not save your follow.');
    } finally {
      setFollowingBusy(current => { const copy = new Set(current); copy.delete(targetId); return copy; });
    }
  }
  ...
  return <SafeAreaView style={s.safe}>
    <Animated.ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={s.scroll} onScroll={onScroll} scrollEventThrottle={16}>
       <View style={s.head}><View><Text style={s.kicker}>YOUR GIRLIES FEED</Text><Text style={s.title}>Hey, girlie ✦</Text></View><View style={s.headIcons}><Pressable onPress={() => router.push('/notifications')}><I name="bell" size={25} /><View style={s.dot} /></Pressable><Pressable onPress={() => router.push('/profile')}><Avatar size={42} /></Pressable></View></View>
       {loadError ? <View style={s.loadError}><Text style={s.loadErrorText}>{loadError}</Text><Pressable onPress={() => setReloadToken(value => value + 1)}><Text style={s.retry}>Try again</Text></Pressable></View> : null}
      ...
      <SectionTitle title="Popular sellers" />
        <ScrollView horizontal showsHorizontalScrollIndicator={false}>{stores.map(store => <Pressable key={store.id} onPress={() => router.push('/seller/' + store.id)} style={s.seller}><Avatar size={54} uri={store.owner?.avatar_url} /><View style={s.sellerNameRow}><Text style={s.sellerName} numberOfLines={1}>{store.name}</Text>{(store.owner?.verified || store.verification_status === 'verified') && <VerifiedMark size={17} />}</View><Text style={s.sellerMeta}>{Number(store.owner?.followers_count || 0).toLocaleString()} followers</Text>{store.owner_id ? <Pressable onPress={event => { event.stopPropagation(); void toggleSeller(store.owner_id); }} style={s.follow}><Text style={s.followText}>{followed.has(store.owner_id) ? 'Following' : 'Follow'}</Text></Pressable> : null}</Pressable>)}</ScrollView>
      <SectionTitle title="Popular products" />
       <ScrollView horizontal showsHorizontalScrollIndicator={false}>{products.slice(0, 5).map(product => <ProductCard key={product.id} productId={product.id} name={product.name} price={`${product.currency === 'GHS' ? 'GH₵' : product.currency} ${Number(product.price).toFixed(0)}`} image={product.image_urls[0] || ''} seller={product.store?.name || 'Girlies seller'} verified={Boolean(product.store?.owner?.verified || product.store?.verification_status === 'verified')} onPress={() => router.push({ pathname: '/product', params: { id: product.id } })} />)}</ScrollView>
