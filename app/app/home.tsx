...
export default function Home() {
  ...
  const banners = [
    { image: heroGirlImage, imageFit: 'contain' as const, copyTop: true, title: 'Find your next look.', subtitle: 'Fresh looks and new\ndrops.', tag: 'NEW SEASON', color: C.rose },
    { image: studyBannerImage, imageFit: 'contain' as const, overlay: true, copyAlign: 'center' as const, tag: '', textColor: '#FFF', title: 'Make time for you.', subtitle: 'Little rituals. Big glow.', color: C.plum },
    { image: skincareBannerImage, imageFit: 'contain' as const, overlay: true, showTagOnly: true, textColor: '#FFF', title: 'Your glow, your way.', subtitle: 'Simple care for every day.', tag: 'THE GIRLIE DROP', color: C.rose },
  ];
  ...
  return <SafeAreaView style={s.safe}>
    <Animated.ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={s.scroll} onScroll={onScroll} scrollEventThrottle={16}>
      ...
      <View style={[s.editorial, { backgroundColor: '#FFD861' }]}><Image source={guideImage} style={s.editorialImg} resizeMode="cover" /><View style={s.editorialText}><Text style={s.editorialTitle}>Good taste is better when shared.</Text><Text style={s.editorialSub}>Save a look. Ask the community. Find your style.</Text><Pressable onPress={() => router.push('/community')} style={s.darkBtn}><Text style={{ color: '#FFF', fontWeight: '900' }}>See what girls are saying</Text></Pressable></View></View>
      ...
      <View style={s.mini}><Text style={s.miniTitle}>Find your next event.</Text><Text style={s.miniText}>Explore upcoming events in the community and get your ticket.</Text><Pressable onPress={() => router.push('/shop/category/events')}><Text style={s.miniLink}>Browse events →</Text></Pressable></View>
